...
--]]
local ChatExpression = import("app.config.ChatExpression")
...
function TalkLabel:createLabel(info)
    ...
    for i,v in ipairs(list) do
        ...
        if a > 0 then
            if i < a+3 then
            elseif i == a+3 then -- pad_bg
                for k,v_ in pairs(ChatExpression) do
                    if expTable[b].text == v_.Name then
                        local sprite = display.newSprite("Chat_Bg_Self.png")
...
function TalkLabel:showText()
    ...
    local alllines = self:adjustLineBreak_(self.textSprite, self.spacing)
