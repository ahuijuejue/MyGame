...
local RichLabelSenior = class("RichLabelSenior", function()
...
RichLabelSenior.__index      = RichLabelSenior
RichLabelSenior._fontName = nil
RichLabelSenior._fontSize = nil
RichLabelSenior._fontColor = nil
RichLabelSenior._containLayer = nil --装载layer
RichLabelSenior._spriteArray = nil --精灵数组
RichLabelSenior._textStr = nil
RichLabelSenior._maxWidth = nil
RichLabelSenior._maxHeight = nil

--播放状态 1 表示未开始 2 表示播放中 3 表示已经播放完毕
RichLabelSenior._labelStatus = 1
RichLabelSenior._tag = {
	fontColor = 1,
	fontSize = 2,
	scale = 3,
	image = 4,
...
function RichLabelSenior:create(params)
	local ret = RichLabelSenior.new()
	ret:init_(params)
	return ret
end

--设置尺寸
function RichLabelSenior:setDimensions(dimensions)
...
--获得label尺寸
function RichLabelSenior:getLabelSize()
...
-- --是否在播放动画
-- function RichLabelSenior:isRunningAmim()
...
-- --强制停止播放动画
-- function RichLabelSenior:playEnded()
-- 	print("play ended")
...
-- --播放fade in 动画
-- function RichLabelSenior:playFadeInAnim(wordPerSec)
...
function RichLabelSenior:init_(params)
	--如果text的格式指定字体则使用指定字体，否则使用默认字体
	--大小和颜色同理
	local fontName   = params.fontName or display.DEFAULT_TTF_FONT --默认字体
	...
	containLayer:setTouchSwallowEnabled(false)
...
--设置text
function RichLabelSenior:setLabelString(text)
...
--获得每个精灵的尺寸
function RichLabelSenior:getSizeOfSprites_(spriteArray)
	local widthArr = {} --宽度数组
...
--获得每个精灵的位置
function RichLabelSenior:getPointOfSprite_(widthArr, heightArr, dimensions)
...
--调整位置（设置文字和尺寸都会触发此方法）
function RichLabelSenior:adjustPosition_()
...
--创建精灵
function RichLabelSenior:createSprite_(parseArray)
	local spriteArray = {}
	...
	for i, dic in ipairs(parseArray) do
		...
		if #textArr > 0 then --创建文字
			...
			for j, word in ipairs(textArr) do
				local param = {text = word,size = fontSize,font = fontName}
...
--将字符串转换成一个个字符
function RichLabelSenior:formatString_(parseArray)
	for i,dic in ipairs(parseArray) do
		local text = dic.text
		if text then
...
function RichLabelSenior:parseString_(str)
		...
		string.gsub(ns, string.sub(ns, 2, #ns-1), function (w)
			...
			if n then
				...
				for k,pstr in pairs(temTab) do
					...
					if RichLabelSenior._tag[pname] and js ~= "" then
						if k == 1 then
							tStr = pname
						end
	...
	-- 普通格式label显示
	if table.nums(clumpheadTab) == 0 or table.nums(totalTab) == 0 then
		local ptab = {}
		ptab.text = str
...
--[[解析16进制颜色rgb值]]
function  RichLabelSenior:convertColor_(xStr)
...
-- string.split()
function RichLabelSenior:stringSplit_(str, flag)
	local tab = {}
	while true do
...
-- 拆分出单个字符
function RichLabelSenior:stringToChar_(str)
    local list = {}
