...
local UnionSelectLayer = class("UnionSelectLayer", function()
	return display.newNode()
end)

function UnionSelectLayer:setSuperCount(count)
...
function UnionSelectLayer:ctor(options)
	self:initData(options)
	self:initView()
	self:setNodeEventEnabled(true)
end

function UnionSelectLayer:initData(options)
	options=options or {}
	-- 位置上限
...
function UnionSelectLayer:initView()
	...
	:addButtons({
		...
		base.Grid.new({normal = "Ready_Label_Normal.png", selected = "Ready_Label_Selected.png"}):addLabel({text="雇佣兵", size=22}),
	})
...
function UnionSelectLayer:getCanUnselected()
	return self.canUnseleted
end

function UnionSelectLayer:setCanUnselected(b)
	self.canUnseleted = b
end

function UnionSelectLayer:selectedList(event)
	...
	if item:isSelected() then
		...
		if not self:isContain(data) then
			if self:canSelected() then
			 	self:selectedGetList(data, item)
				item:setSelected(true)
			end
		else
			showToast({text = "不能重复上场同一个英雄！！"})
		end
	end
end

function UnionSelectLayer:addHeroPos(heroIndex, x, y, zorder)
...
function UnionSelectLayer:createHeroPos(index, heroTitleImgName)
...
function UnionSelectLayer:createDisHeroPos(index)
	local grid = base.Grid.new()
	:setIndex(index)
...
-- 点击选中的 图像 移动
function UnionSelectLayer:movedGetGrid(index, x, y)
	if index > #self.getDataList_ then
		return
	end
...
-- 点击选中的 图像 移动结束
function UnionSelectLayer:endedGetGrid(index, x, y)
...
-- 选中区 交换英雄
function UnionSelectLayer:didExchangeGet(idx1, idx2)
...
-- function UnionSelectLayer:sortHeroData(index)
-- 	local types = {0,1,2,3,0}
...
function UnionSelectLayer:unselectedCacheList(data)
	local index = table.indexof(self:getCurrentList(), data)
...
function UnionSelectLayer:selectedGetListIndex(index)
	local data = self:getCurrentList()[index]
	local item = self.listView_.items_[index]:getContent()
 	self:selectedGetList(data, item)
	item:setSelected(true)
...
-- 是否有重复英雄上阵
function UnionSelectLayer:isContain(hero)
	for i,v in ipairs(self.getDataList_) do
		if v.roleId == hero.roleId then
			return true
		end
	end
	return false
...
function UnionSelectLayer:selectedGetList(data, fromGrid)
...
function UnionSelectLayer:unselectedGetList(data)
...
function UnionSelectLayer:showMoving(imgName, fromPos, toPos, callback)
...
function UnionSelectLayer:showGrid(grid, data)
...
function UnionSelectLayer:showGridEx(grid, data)
	--
end

function UnionSelectLayer:gridAddName(grid, data)
	local nameLayer = display.newNode():zorder(2)

	display.newSprite("Ready_Name_Banner.png"):addTo(nameLayer)
	local nameText = ""
	if data.teamId and data.teamId ~= UserData.teamId then
		nameText = data.teamName
	else
		nameText = data.name
	end
	base.Label.new({text = nameText, size = 18}):align(display.CENTER):addTo(nameLayer)
	grid:addItemsWithKey({
		name_ = nameLayer:zorder(3):pos(0, -60),
	})
end

function UnionSelectLayer:showGridGet(grid, data)
	local typeImg = self:getHeroTypeImage(data.roleId)
	grid:addItemsWithKey({
...
function UnionSelectLayer:updateBattle()
	local battle = 0
	for i,v in ipairs(self.getDataList_) do
...
function UnionSelectLayer:setHeroList(list)
...
function UnionSelectLayer:getHeroList()
	local list = {}
	local listData = {
		team1 = {},
		team2 = {},
...
function UnionSelectLayer:getHeroCount()
	return #self.getDataList_
end

function UnionSelectLayer:onTouch(listener)
	self.listener_ = listener
	return self
end

function UnionSelectLayer:listenerEvent_(event)
...
function UnionSelectLayer:updateData()
	self:updateHeroData()
	self:updateHeroGetData()
end

function UnionSelectLayer:sortHeroList(list)
...
function UnionSelectLayer:updateHeroData()
	self.data_ = {}
	local list = {LIST_TYPE.HERO_ALL, LIST_TYPE.HERO_TANK, LIST_TYPE.HERO_DPS, LIST_TYPE.HERO_AID, LIST_TYPE.HERO_AGENT}
	for i,v in ipairs(list) do
		if v == LIST_TYPE.HERO_ALL then
			local heros = HeroListData:getListWithType(v)
			local heros_ = HeroListData:getListWithType(LIST_TYPE.HERO_AGENT)
			for i1,v1 in ipairs(heros_) do
				table.insert(heros, v1)
			end
...
function UnionSelectLayer:updateHeroGetData()
...
function UnionSelectLayer:updateListView()
	self.listView_
	:removeAllItems()
...
function UnionSelectLayer:onEnterTransitionFinish()
	self:updateData()
	self:updateView()

end

function UnionSelectLayer:onExit()
	self.heroList_ = self:getHeroList()
end

function UnionSelectLayer:getHeroTypeImage(id)
	local cfg = GameConfig.Hero[id]
	return cfg.HeroJob
end

return UnionSelectLayer
