...
local TrialHouseReadyScene = class("TrialHouseReadyScene", base.Scene)

function TrialHouseReadyScene:initData(options)
    self.grade = options.grade or 1     
    self.heroList_ = UserData:getHouseBattleList()
end

function TrialHouseReadyScene:initView()
    self:autoCleanImage()
    -- 背景
...
function TrialHouseReadyScene:didStartGame()
    local heroId, heroData = self.selectLayer_:getHeroList()  
    UserData:setHouseBattleList(heroId)

    local data = TrialData:getHouse(self.grade) 
    ...
    local toData = {
        ...
        wave               = table.concat(data:getDataList(), ","),     -- 波 
        type               = data.type,                            -- 类型。1.物理 2.魔法
        secondStarTime     = 90,    
        ...
        item               = {},
        levelId            = data.id,
        alive1             = data.reward[1],
        alive2             = data.reward[2],
        alive3             = data.reward[3],
        alive4             = data.overReward,
        tailSkill          =   {                   
    ...
    -- 进入战斗界面
    app:enterScene("TrialHouseBattleScene", {toData})
    
end


return TrialHouseReadyScene
