...
local BoxLayout = class("BoxLayout", UIBoxLayout)

function BoxLayout:ctor(direction, name)
	BoxLayout.super.ctor(self, direction, name)
end

local depth_ = 0

function BoxLayout:apply(container)
    ...
    for widget, v in pairs(self.widgets_) do
        ...
        if widgetSizePolicyV == display.FIXED_SIZE then
            fixedHeight = fixedHeight + widgetSizeHeight + marginTop + marginBottom
    ...
    local top = fixedHeight + containerPaddingBottom
    local right = fixedWidth + containerPaddingLeft
