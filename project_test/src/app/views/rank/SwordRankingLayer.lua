...
local SwordRankingLayer = class("SwordRankingLayer", function()
	return display.newNode()
end)

function SwordRankingLayer:ctor()
	print("战斗力排名")
	self:initData()
	self:initView()
end

function SwordRankingLayer:initData()

end

function SwordRankingLayer:initView()
	...
	self:addWord("word_20_sword.png", 470, theight)
...
function SwordRankingLayer:addWord(name, x, y)
	display.newSprite(name)
	:addTo(self)
	:pos(x, y)
end

function SwordRankingLayer:selectedIndex(index)
	local teamData = self.data[index]
	NetHandler.gameRequest("ShowTeamDefenseInfo", { param1 = teamData.userId ,param2 = 3, param3 = teamData.score})
end

function SwordRankingLayer:updateData()
	self:updateListData()
end

function SwordRankingLayer:updateView()
	self:updateListView()
end


function SwordRankingLayer:updateListData()
...
function SwordRankingLayer:updateListView()
	self.listView_
	:resetData()
